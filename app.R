...
#load in data
CEPRdata <- read_rds("CEPR_reduced.rds")
        ...
        #Second histogram type for continuous variables
           ...
           plot_selected <- CEPRdata %>% 
               ...
               ggplot(aes_string(x = input$x, 
                              fill = input$fill)) +  
               geom_histogram() +
             geom_vline(aes(xintercept = mean(get(input$x))), fill = "red") +
             geom_vline(aes(xintercept = median(get(input$x))), fill = "blue", linetype = "dashed")
...
#Remainder plots don't make sense, but going to allow them to be user created anyways  
    ...
    output$dynamicTable <- renderTable({
      ...
      #Stacked bar graphs, separable   
        ...
        CEPRdata %>%
          group_by_(input$x, input$y, input$fill) %>%
          summarise(Count = n()) %>%
          mutate(Proportion = Count / sum(Count))
      }
      tabulate
